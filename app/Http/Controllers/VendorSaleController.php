...
class VendorSaleController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            if ($request->id)
            $purchase_credit = PurchaseCredit::where('payment_id',$sale->id)->first();
            else
            $purchase_credit = new PurchaseCredit;

            $purchase_credit->user_id = Auth::user()->id;
            $purchase_credit->vendor_id = $sale->customer_id;
            $purchase_credit->payment_id = $sale->id;
            $purchase_credit->amount = - $sale->pay_amount;
            $purchase_credit->remarks = "AMOUNT AGAINST VENDOR SALE";
    ...
    {
            $sales =  Sale::with('items', 'vendor','items.product')->where('type',2);
